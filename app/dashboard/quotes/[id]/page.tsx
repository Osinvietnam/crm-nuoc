...
export default function QuoteDetailPage() {
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      // Chuyển sang BG mới
      setTimeout(() => router.replace(`/dashboard/quotes/${data.data.record_id}`), 1200)
    } catch { notify('Lỗi kết nối', true) }
  ...
  if (!quote) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-3">
      ...
      <button onClick={() => router.push('/dashboard/quotes')} className="text-blue-600 text-sm font-semibold">← Quay lại</button>
    </div>
  )
